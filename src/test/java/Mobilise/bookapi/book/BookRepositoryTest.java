...
class BookRepositoryTest {
    ...
    void itShouldFindAllBooksByAuthorId() {
        ...
        Author author = Author.builder().id(authorId).specialization("horror").dateOfBirth(LocalDate.now()).user(user)
                .build();

        Set<Author> authors = new HashSet<Author>();
        authors.add(author);

        Book book1 = Book.builder().id(bookId).title("Book 1").description("Book 1 Description").publicationYear(2020)
                .authors(authors).build();
        Book book2 = Book.builder().id(bookId).title("Book 2").description("Book 2 Description").publicationYear(2021)
                .authors(authors).build();
